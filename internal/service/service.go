...
func (m *Manager) getServiceInfo(name string) (*ServiceInfo, error) {
	composePath := filepath.Join(m.ServicesDir, name, "docker-compose.yml")
	data, err := os.ReadFile(composePath) //nolint:gosec // path constructed from trusted config
	if err != nil {
...
func (m *Manager) containerStatus(name string) (status, health, uptime string) {
	...
	cmd := exec.Command("docker", "compose", "-p", "jib-svc-"+name, "-f", "docker-compose.yml", "ps", "--format", "{{.State}}|{{.Health}}|{{.RunningFor}}") //nolint:gosec // args constructed from trusted config
	cmd.Dir = svcDir
	out, err := cmd.CombinedOutput()
	if err != nil {
		// Try simpler format
		cmd2 := exec.Command("docker", "compose", "-p", "jib-svc-"+name, "-f", "docker-compose.yml", "ps", "--format", "{{.State}}") //nolint:gosec // args constructed from trusted config
		cmd2.Dir = svcDir
...
func (m *Manager) loadCredentials(name string) (map[string]string, error) {
	secretFile := filepath.Join(m.SecretsDir, name+".env")
	data, err := os.ReadFile(secretFile) //nolint:gosec // path constructed from trusted config
	if err != nil {
		return nil, err
	}
...
func (m *Manager) generateCompose(name, serviceType string, tmpl serviceTemplate, tag, password string) error {
	...
	if err := os.WriteFile(composePath, append(header, data...), 0o600); err != nil {
...
func (m *Manager) composeUp(name string) error {
	svcDir := filepath.Join(m.ServicesDir, name)
	cmd := exec.Command("docker", "compose", "-p", "jib-svc-"+name, "-f", "docker-compose.yml", "up", "-d") //nolint:gosec // args constructed from trusted config
	cmd.Dir = svcDir
...
func (m *Manager) composeDown(name string, volumes bool) error {
	...
	cmd := exec.Command("docker", args...) //nolint:gosec // args constructed from trusted config
	cmd.Dir = svcDir
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	return cmd.Run()
...
func ensureSharedNetwork() error {
	...
	cmd := exec.Command("docker", "network", "inspect", "jib-shared") //nolint:gosec // trusted CLI subprocess
	if err := cmd.Run(); err == nil {
		return nil // already exists
	}

	// Create it
	cmd = exec.Command("docker", "network", "create", "jib-shared") //nolint:gosec // trusted CLI subprocess
	out, err := cmd.CombinedOutput()
	if err != nil {
...
func (m *Manager) WaitHealthy(name string, timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		cmd := exec.Command("docker", "compose", "-p", "jib-svc-"+name, "-f", "docker-compose.yml", "ps", "--format", "{{.Health}}") //nolint:gosec // args constructed from trusted config
		cmd.Dir = svcDir
		out, err := cmd.CombinedOutput()
		if err == nil {
