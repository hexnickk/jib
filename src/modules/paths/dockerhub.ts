...
export function pathsDockerHubImage(repo: string): string | null {
  if (repo.startsWith('docker://')) {
    return normalizeImage(repo.slice('docker://'.length))
  }
  if (repo.startsWith('dockerhub://')) {
    return normalizeImage(repo.slice('dockerhub://'.length))
  }
  ...
  if (parts[0] === 'r' && parts[1] && parts[2]) {
    return normalizeImage(`${parts[1]}/${parts[2]}`)
  }
  if (parts[0] === '_' && parts[1]) {
    return normalizeImage(parts[1])
  }
  return null
}
