...
test('txRunSteps > runs steps in order without rollback on success', async () => {
  const calls: string[] = []
  ...
  const result = await txRunSteps({}, steps, {
...
test('txRunSteps > rolls back completed steps in reverse order after a failure', async () => {
  const calls: string[] = []
  const steps: Step<object, string, InternalError>[] = [
    {
      name: 'one',
...
test('txRunSteps > returns cancellation and rolls back completed steps when cancelled mid-flow', async () => {
  const calls: string[] = []
  ...
  const result = await txRunSteps({}, steps, {
    signal,
    cancelled: () => new CancelledError('cancelled'),
  })
...
test('txRunSteps > warns and keeps rolling back when a rollback step fails', async () => {
...
test('txRunSteps > warns for an already-typed rollback failure', async () => {
  const warnings: string[] = []
  const steps: Step<object, string, InternalError>[] = [
    {
      name: 'one',
