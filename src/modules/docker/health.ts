...
export async function dockerCheckHealth(
  ...
  for (const check of checks) {
    results.push(await probe({ fetchFn, sleep, intervals, requestTimeoutMs }, check))
  }
  return results
}
...
async function probe(
  ...
  const { fetchFn, sleep, intervals, requestTimeoutMs } = ctx
  ...
  for (let attempt = 0; attempt < intervals.length; attempt++) {
    ...
    if (attempt < intervals.length - 1) {
      await sleep(intervals[attempt] ?? 0)
    }
  }
  const out: HealthResult = { endpoint, ok: false }
  if (lastStatus !== undefined) {
    out.statusCode = lastStatus
  }
