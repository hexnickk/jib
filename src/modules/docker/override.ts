 ...
 * write it to `<overrideDir>/<app>.yml`. Returns its absolute path or a typed
 * filesystem error; a partial temporary file is removed on failure.
 */
export async function dockerWriteOverride(
  ...
  try {
    ...
    return new InternalError(`write Docker override: ${message}`, { cause: error })
  }
}
