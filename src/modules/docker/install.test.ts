...
test('docker install helpers > dockerParseOsRelease parses quoted values', () => {
...
test('docker install helpers > dockerSelectAptRepository selects Ubuntu for derivatives with UBUNTU_CODENAME', () => {
  expect(
    dockerSelectAptRepository({
      ID: 'linuxmint',
...
test('docker install helpers > dockerEnsureInstalledResult skips package install when Docker is already ready', async () => {
  const calls: string[] = []
...
test('docker install helpers > dockerEnsureInstalledResult installs Docker from apt when runtime pieces are missing', async () => {
  const calls: string[] = []
  let installed = false
  let source = ''
...
test('docker install helpers > dockerEnsureInstalledResult returns a typed error when systemd is unavailable', async () => {
...
test('docker install helpers > dockerEnsureInstalledResult reports os-release read failures accurately', async () => {
  const error = await dockerEnsureInstalledResult({
    readOsRelease: async () => {
