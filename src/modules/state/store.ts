...
// Approved exception to the ctx convention: pass the state directory directly.
function statePath(stateDir: string, app: string): string {
  return join(stateDir, `${app}.json`)
}

/** Loads one app's state file, returning a typed error on read/parse failures. */
export async function stateLoad(stateDir: string, app: string): Promise<AppState | InternalError> {
  const path = statePath(stateDir, app)
  let raw: string
  try {
...
export async function stateSave(
  ...
  const target = statePath(stateDir, app)
...
export async function stateRecordFailure(
  ...
  const state = await stateLoad(stateDir, app)
  ...
  return stateSave(stateDir, app, state)
}
