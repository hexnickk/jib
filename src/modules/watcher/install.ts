...
> {
  ...
  const vars = { jibRoot: ctx.paths.root, binPath: deps.binPath ?? watcherCurrentBinPath() }
...
/** Resolves the current CLI entrypoint for systemd units generated by npm installs. */
function watcherCurrentBinPath(): string {
  return process.argv[1] ?? '/usr/local/bin/jib'
}
