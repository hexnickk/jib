...
test('cloudflared install/uninstall > cloudflaredInstallResult writes managed files and triggers daemon reload', async () => {
  const calls: string[] = []
...
test('cloudflared install/uninstall > cloudflaredUninstallResult disables the service, removes managed files, and reloads systemd', async () => {
  const calls: string[] = []
  const root = await mkdtemp(join(tmpdir(), 'jib-cloudflared-'))
  const paths = pathsGetPaths(root)
  ...
  try {
    ...
    await writeFile(join(paths.cloudflaredDir, 'docker-compose.yml'), 'services:\n  cloudflared:\n')
...
test('cloudflared install/uninstall > cloudflaredInstallResult returns a typed reload error for non-zero daemon-reload exits', async () => {
  const root = await mkdtemp(join(tmpdir(), 'jib-cloudflared-'))
  const paths = pathsGetPaths(root)
...
test('cloudflared install/uninstall > cloudflaredInstallResult returns typed unit write errors', async () => {
  const root = await mkdtemp(join(tmpdir(), 'jib-cloudflared-'))
  const paths = pathsGetPaths(root)
...
test('cloudflared install/uninstall > cloudflaredUninstall keeps cleaning up when disable reports an absent unit', async () => {
  const root = await mkdtemp(join(tmpdir(), 'jib-cloudflared-'))
  const paths = pathsGetPaths(root)
  const calls: string[] = []
...
test('cloudflared install/uninstall > cloudflaredUninstallResult returns a typed disable error after cleanup on real failures', async () => {
  const root = await mkdtemp(join(tmpdir(), 'jib-cloudflared-'))
  const paths = pathsGetPaths(root)
  const calls: string[] = []
...
test('cloudflared install/uninstall > cloudflaredUninstallResult returns a typed reload error for non-zero daemon-reload exits', async () => {
  const root = await mkdtemp(join(tmpdir(), 'jib-cloudflared-'))
  const paths = pathsGetPaths(root)
...
test('cloudflared install/uninstall > cloudflaredUninstallResult returns uninstall errors for thrown disable failures', async () => {
  const root = await mkdtemp(join(tmpdir(), 'jib-cloudflared-'))
  const paths = pathsGetPaths(root)
