...
async function withTmpRoot<Value>(fn: (root: string) => Promise<Value>): Promise<Value> {
...
test('execution contract > non-interactive add without app returns text errors', async () => {
  await withTmpRoot(async (root) => {
    ...
    expect(result.stderr).toContain('app: provide <app> or rerun with interactive prompts enabled')
    expect(result.stderr).not.toContain('stack')
  })
})

test('execution contract > version renders text', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > help renders text', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > root invocation renders help instead of failing', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > exec accepts passthrough arguments instead of rejecting the app positional', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > run accepts passthrough arguments instead of rejecting the app positional', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > exec reports missing passthrough command before app lookup', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > env set dispatches by action name', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > env list dispatches by action name', async () => {
  await withTmpRoot(async (root) => {
    await writeDemoAppConfig(root)
...
test('execution contract > status renders apps as labeled text blocks', async () => {
  await withTmpRoot(async (root) => {
    expect(
      await configWrite(join(root, 'config.yml'), {
        config_version: 3,
...
test('execution contract > unknown command prints help and fails', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > invalid root runtime flag prints help instead of crashing', async () => {
  await withTmpRoot(async (root) => {
...
test('execution contract > output mode flag is not registered', async () => {
  await withTmpRoot(async (root) => {
