...
test('optional module configuration > initPersistModuleChoice preserves unrelated config edits', async () => {
...
test('optional module configuration > initPersistModuleChoice returns a typed error when persistence fails', async () => {
...
test('optional module configuration > initConfigureOptionalModules preserves setup-written config before enabling the module', async () => {
...
test('optional module configuration > initConfigureOptionalModules rolls back install when setup does not complete', async () => {
  await withTmpConfig(async (cfg, root) => {
    const paths = pathsGetPaths(root)
    const calls: string[] = []
    const mods: ModLike[] = [
...
test('optional module configuration > initConfigureOptionalModules does not enable a module when setup does not complete', async () => {
  await withTmpConfig(async (cfg, root) => {
    const paths = pathsGetPaths(root)
...
test('optional module configuration > initConfigureOptionalModules keeps earlier choices when a later setup fails', async () => {
  await withTmpConfig(async (cfg, root) => {
    const paths = pathsGetPaths(root)
...
test('optional module configuration > initConfigureOptionalModules converts thrown install dependency failures', async () => {
  await withTmpConfig(async (cfg, root) => {
    const paths = pathsGetPaths(root)
    const mods: ModLike[] = [
      {
...
test('optional module configuration > initConfigureOptionalModules returns typed setup errors', async () => {
  await withTmpConfig(async (cfg, root) => {
    const paths = pathsGetPaths(root)
    const mods: ModLike[] = [mod('cloudflared')]
