...
test('guided add helpers > single-service web app can collect a domain and scoped config', () => {
...
test('guided add helpers > multi-service app merges repeated config keys across scopes', () => {
  const merged = addMergeGuidedServiceAnswers(
...
test('guided add helpers > worker-only app remains valid without ingress', () => {
  const merged = addMergeGuidedServiceAnswers(
    [],
    ['worker'],
...
test('guided add helpers > multi-service cli domains must name a service in non-interactive mode', () => {
...
test('guided add helpers > detectedConfigScopes merges runtime and build references for the same key', () => {
  expect(
    addDetectedConfigScopes({
      name: 'web',
      publishesPorts: true,
...
test('guided add helpers > summary renders services plus runtime and build config expectations', () => {
...
test('guided add helpers > env entry parser accepts KEY=VALUE format and rejects missing equals', () => {
...
test('guided add helpers > comma splitter treats missing optional prompt input as empty', () => {
